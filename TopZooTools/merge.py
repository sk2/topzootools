...
class FileGetter(threading.Thread):
    ...
    def run(self):
        ...
        if "http://ws.geonames.org/get" in self.url:
            # geoname_id result
...
def geocode_sql(country=None, continent=None, conn=None,
                country_info=None):
    if conn is None:
        return

    #TODO: mvoe this back into the main function, split out later
...
#TODO: merge geocode_sql with this

def country_code(graph, sql_conn = None, country_info=None):
    country_name = None
    country_code = None
...
def geocode(graph, sql_conn=None, web_cache=None, country_info=None):
    # How fuzzy we accept results to be [0,1], lower = accept 'fuzzier' answers
    #TODO: pass this on to appropriate functions
    fuzzy = 0.8

    c_code = country_code(graph, sql_conn, country_info)

    def create_url(n, iteration):
        ...
        if 'geocode_id' in graph.node[n]:
            # use this instead of country code
            params['geonameId'] = graph.node[n]['geocode_id']
            params['style'] = "SHORT"
        ...
        if i == 0:
            ...
            if 'geocode_extent' in graph.node[n]:
                # use this instead of country code
                city_extent = city + ' ' + graph.node[n]['geocode_extent']
                params['q'] = city_extent
            elif 'geocode_append' in graph.node[n]:
                # use this instead of country code
                city_appended = city + ' ' + graph.node[n]['geocode_append']
    ...
    for n, data in geocoded.items():
        if data:
            graph.node[n]['Latitude'] =  float(data['lat'])
            graph.node[n]['Longitude'] = float(data['lng'])
        else:
            logger.info( "No match found for " + graph.node[n]['label'])


    if options.check_geocode:
        graph = check_geocode(graph, country_code = c_code)
    ...
    for n,data in graph.nodes(data=True):
        ...
        if 'geocode_extent' in data:
            del graph.node[n]['geocode_extent']
...
def main():
    ...
    if (os.path.isfile(cache_file)):
        ...
        web_cache = cPickle.load(f)
        f.close()

    # read in country info
    #TODO: add pickle support for speed
    #TODO if country_info pickle not exist, then fetch from web
    country_info = {}
    #TODO: see if faster to create multiple dictionaries for lookup
    country_info_file = 'country_info.data'
    if (os.path.isfile(country_info_file)):
        f = open(cache_file, 'rb')
        country_info = cPickle.load(f)
    else:
        #TODO: make download from geonames website
        csv_file = open( '/Users/sk2/zoo/countryInfo.txt', "rU" )
    ...
    for source_file in network_files:
        ...
        #if network_files.index(source_file) >= 6:
        #    break
        ...
        if skip_net:
            logger.info("Skipping {0} as not tagged".format(net_name))
            continue
        ...
        if options.geocode:
            ...
            graph = geocode(graph, sql_conn, web_cache, country_info)
            f = open(cache_file, 'wb')
            cPickle.dump(web_cache, f, -1)
            f.close()
            # and write back the cache
